...
// POST /api/video
// Body: { prompt: string, negativePrompt?: string, aspectRatio?: '16:9'|'9:16', resolution?: '720p'|'1080p', personGeneration?: 'allow_all'|'allow_adult'|'dont_allow', imageUrl?: string }
router.post('/', async (req, res) => {
  try {
    if (!gemini) gemini = new GeminiService();
    const { prompt, negativePrompt, aspectRatio, resolution, personGeneration, imageUrl } = req.body || {};
