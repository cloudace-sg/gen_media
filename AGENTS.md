...
- **Custom Search API**: Requires the Programmable Search Engine (`cx`) to be owned by the same Google account that owns the API key's GCP project. A 403 error with "This project does not have the access to Custom Search JSON API" usually means a project/owner mismatch, not a missing API enablement.
