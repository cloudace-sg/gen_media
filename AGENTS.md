...
- **Search API (Pexels)**: Image and video reference search uses the Pexels API (free, no quota issues). Set `PEXELS_API_KEY` in `server/.env`. The legacy Google Custom Search JSON API was deprecated in 2026 and is closed to new customers. Image search: `GET /api/search?query=...`. Video search: `GET /api/search/videos?query=...`.
